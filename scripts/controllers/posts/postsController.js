...
dom.loadingMoreArticlesBtn.addEventListener('click', async () => {
  await loadTemplate('../../../templates/loading.html');
  loading();
  try {
    buildPosts(4, true);
...
async function buildPosts(size, loadingMoreArticles) {
  const posts = await PostService.findAllPostsPageableByStatus(
    MediaTypes.JSON, 
    { page: currentPageNumber, size: size, direction: 'asc' }, 
    PostStatus.PUBLISHED
  );

  if (currentPageNumber+1 < posts.page.totalPages) {
    dom.loadingMoreArticlesBtn.style.display = 'initial';
  }
  else {
    dom.loadingMoreArticlesBtn.style.display = 'none';
  }

  if (posts._embedded === undefined) throw new Exceptions.TheListIsEmptyException();

  loadingMoreArticles 
    ? await renderArticlesGrid(posts._embedded.postDTOList)
